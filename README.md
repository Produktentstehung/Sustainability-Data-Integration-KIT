...
The flow „PLM” provides the functionality to retrieve part and document data from the PLM system and use this data to generate an Asset Administration Shell (AAS). The flow is triggered by the “PLM Flow (Master)” and first extracts the relevant part information, including the part name, article number, ERP number, status, navigation ID, quantity, unit, category, weight, material, and the PLM UI link. The flow also retrieves the URL of the related documents from the part data.
